...
export default function DeskEdit() {
  const tabs = [
    ...
    { 
      ...
      content: 
      <Floor>
        <Image src={floo1} className='h-full w-full bg-cover bg-center  relative' alt="" />
      </Floor>
    },
    { 
      title: 'floor 2', 
  ...
  return (
    <div className=" h-full w-full mx-auto p-4">
      ...
      {/* adddesk */}
       <Button className='bg-[#606DE3] px-4 h-9 text-sm hover:bg-[#9292DD] gap-2' onClick={handleShowDesk}>
        <GiDesk className='text-xl'/>
          Add Desk
       </Button>
       <Button className='bg-[#606DE3] px-4 h-9 text-sm hover:bg-[#9292DD] gap-2' onClick={handleAddFloor}>
      ...
      //  add desk
      <div className='h-[530px] w-full flex flex-col justify-center items-center pt-1'>
        <div className={`h-full w-full flex flex-row ${showDeskContainer? ' gap-10 justify-center' : 'items-center justify-center'}`}> 
          <div className={` ${showDeskContainer? 'w-[25rem] relative' : 'w-full flex justify-center items-center'}`}>
            <div className={` ${showDeskContainer? 'w-[25rem] h-[18.75rem] drop-shadow-md' : 'w-[37.5rem] h-[28.125rem] flex justify-center'} drop-shadow-md`}>
              {tabs[activeTab].content} 
            </div>
              
              {/* for Department */}
             <div className={`absolute w-full h-[190px] bg-white rounded-md bottom-0 right-0 drop-shadow-md ${!showDeskEdit && "hidden"}`}>
             ...
             <form action="">
               <div className='w-full flex flex-col gap-1'>
                ...
                <Select >
                  ...
                  <Button className=' h-[35px] bg-[#6358DC] hover:bg-[#8177f1]'> Add</Button>                
                  </div>
                </div>
                <div className="mt-1">
                  <label htmlFor="name" className="text-sm">Department  Name</label>
                  <Input type="text" name="name" placeholder='department name' className="px-2 border bg-white border-black w-[210px] h-[35px]" />
                </div>
                <div className='flex flex-row gap-2 mt-2'>
                  <Button className='bg-[#D54F54] px-4 py-1 rounded-lg h-[35px]'>Cancel</Button>
                  <Button className='bg-[#479B7C] px-4 py-1 rounded-lg h-[35px]'>Save</Button>
                </div>
               </form>
                <div className='h-full ml-2'>
                 <p className="text-sm text-black">Color</p>
                 <div className='bg-[#C4C4C4] h-[140px] w-[150px] mt-1'>
          ...
          <div className={` ${showDeskContainer? 'relative h-full w-4/12 flex justify-center ' : 'hidden'}`} >
            {showDeskEdit ? <EditDesk />  : <Desk />}
            <Button className={`bg-[#479B7C] px-3 py-1 rounded-md absolute right-2 bottom-2 ${showDeskEdit && "hidden"}`} onClick={handleClickEditDesk} >Next</Button>
            <Button className={`bg-[#D54F54] px-3 py-1 rounded-md absolute left-2 bottom-2 ${!showDeskEdit && "hidden"}`} onClick={handleClickEditDeskChange} >Back</Button>
          </div>
        </div>
      </div>
